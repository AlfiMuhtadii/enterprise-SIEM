...
| **GAP-003 (audit-tool verification)** | `TenantNullAuditCommand` (GAP-003's Phase 3 audit tool) had zero test coverage and had never been run; added 9 tests + executed the real audit against the dev DB; surfaced and Deferred a new sub-finding (`ISOLATED_TABLES` registry stale vs. actual schema, 23 vs ~156 tenant_id-bearing tables) | `tests/Feature/TenantNullAuditCommandTest.php`, `REVIEW_REJECTED.md` | Medium | [#51](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/51) |
| **UI-COUNTRY-FONT** | Country name in Alert Attribution view was low-contrast (50% opacity) against the dark theme; bumped to an established higher-contrast opacity tier already used elsewhere, kept row font-size consistent | `resources/views/security/attribution.blade.php` | Low | [#52](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/52) |
| **Honeytoken tenant isolation** | `SocHoneytokenController`/`HoneytokenService` had zero tenant scoping despite `honeytokens.tenant_id` existing; also fixed a genuine idempotency bug where repeat scans grew an alert's evidence JSON unboundedly | `app/Http/Controllers/SocHoneytokenController.php`, `app/Services/HoneytokenService.php` | High | [#53](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/53) |
| **SocAgentController tenant isolation** | Endpoint fleet management screen (`endpoint_agents`, already flagged in `ISOLATED_TABLES`) had unscoped reads and no ownership check on policy-assignment/command-queue writes | `app/Http/Controllers/SocAgentController.php` | High | [#54](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/54) |
| **EndpointController tenant isolation** | Read-only twin of the above: `/endpoint` inventory view rendered agent/alert data with no tenant scoping | `app/Http/Controllers/Endpoint/EndpointController.php` | Medium | [#55](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/55) |
| **ARCH-KAFKA-NATIVE + PERF-REST-POLL + PERF-REST-REBALANCE** | Native binary Kafka transport (franz-go) as an opt-in alternative to Pandaproxy REST for the 3 Go pipeline services, off by default; root-caused and structurally eliminates PERF-REST-REBALANCE's rebalance-storm cause; verified against a real in-process fake Kafka broker (kfake); live-pipeline gate remains, blocked on no Docker daemon | `services/{ingestion-gateway,normalizer-worker,correlation-worker}/main.go`, `internal/kafkanative/` | High | [#56](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/56) |
| **ARCH-DB-SPLIT (write path + soak tooling)** | Telemetry write paths (Python ingesters + AgentIngestionController) now route to a new ClickHouse telemetry_events table (tenant_id from day one) behind XDR_TELEMETRY_WRITE_TARGET, off by default; load_test_soc.py gained a real concurrent write-throughput soak generator; read-path migration deliberately deferred as separate, larger scope; live measurement gate remains, blocked on no Docker daemon | `scripts/ingest_telemetry_events.py`, `scripts/telemetry_stream_worker.py`, `app/Services/ClickHouseTelemetryWriter.php` | High | [#57](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/57) |
| **ARCH-DB-SPLIT (live verification)** | Docker became available; ran the write path against real Postgres+ClickHouse for the first time. Found and fixed a real bug (ClickHouse rejects plain ISO-8601 timestamps by default). Captured real evidence: ClickHouse ~29-54% higher write throughput; Postgres dashboard query 3.8x slower (9.4x at p95) under concurrent telemetry write load — the actual empirical claim this backlog item exists to test | `scripts/xdr_infra_clients.py`, `app/Services/ClickHouseTelemetryWriter.php` | High | [#58](https://github.com/AlfiMuhtadii/enterprise-SIEM/issues/58) |
