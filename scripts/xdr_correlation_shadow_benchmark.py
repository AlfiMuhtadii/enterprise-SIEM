...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument("--engine", choices=["legacy", "go", "shadow"], default=os.getenv("XDR_CORRELATION_ENGINE", "shadow"))
...
def gate_status(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    go_count = report["go_correlation"]["alert_count"]
    ...
    gates = {
        ...
        "go_p95_latency_lt_300ms": report["go_correlation"]["p95_latency_ms"] < 300,
...
def main(args: argparse.Namespace | None = None) -> int:
    ...
    if args.scope != "identity-cloud" and args.engine == "go":
        raise SystemExit("ERROR: Go cutover is only allowed for --scope identity-cloud")
    ...
    report: Dict[str, Any] = {
        ...
        "source_of_truth": "go" if effective_engine == "go" else "python_laravel",
        "comparison_engine": "legacy_shadow" if effective_engine == "go" else ("go_shadow" if health["healthy"] else "none"),
        "scope": args.scope,
        "events": len(events),
        ...
        "cutover_monitoring": {
            "alert_count_delta": len(go_alerts) - len(python_alerts),
            "go_p95_latency_ms": percentile([row["latency_ms"] for row in go_runs], 95),
            "go_error_rate": 0 if health["healthy"] else 1,
            "go_worker_health": health,
            "stream_lag": "not measured by local benchmark; use xdr:stream-metrics or Redpanda lag metrics",
            "scope_guard": "endpoint/dns/proxy remain legacy until separate golden, parity, and latency gates pass",
        },
        "cutover_plan": [
            "Default remains shadow.",
            "Use XDR_CORRELATION_ENGINE=go only for XDR_CORRELATION_SCOPE=identity-cloud.",
            "Legacy remains shadow comparison when Go is active.",
            "Fallback to legacy automatically when Go worker is unhealthy and fallback is enabled.",
            "Endpoint/DNS/proxy require separate golden test, large replay parity, and latency gate before expansion.",
        ],
        "pressure": {
    ...
    if fallback:
        report["validation_status"] = "FALLBACK_TO_LEGACY"
    else:
    ...
    print(f"events={len(events)} engine={args.engine} effective={effective_engine} python_alerts={python_metrics['alert_count']} go_alerts={go_metrics['alert_count']} go_eps={go_metrics['throughput_eps']}")
    return 0
