    ...
    for payload in malformed_payloads:
        try:
            ...
            with urllib.request.urlopen(
                req,
                timeout=5,
                context=getattr(args, "internal_ssl_context", None),
            ) as resp:
                status = resp.status
        except urllib.error.HTTPError as e:
            status = e.code
...
def inject_delayed_consumer_simulation(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    """
    status_before, metrics_before = internal_get(
        args, f"{args.normalizer_url}/metrics"
    )
    queue_depth = None
    capacity    = None
    if metrics_before:
...
def validate_replay_after_recovery(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    """
    status, metrics = internal_get(args, f"{args.alert_writer_url}/metrics")
