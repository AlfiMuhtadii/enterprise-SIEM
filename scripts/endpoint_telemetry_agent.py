...
def validate_package_manifest(manifest_path: Path) -> Dict[str, Any]:
    if not manifest_path.exists():
        return {"present": False, "valid": None, "path": str(manifest_path)}
    data = read_json(manifest_path)
    expected = data.get("files") if isinstance(data.get("files"), dict) else {}
    if not expected:
        return {"present": True, "valid": False, "path": str(manifest_path), "error": "missing files map"}
    root = manifest_path.parent
    mismatches = []
    checked = 0
    for rel_path, expected_hash in expected.items():
        actual_hash = file_sha256(root / str(rel_path))
        checked += 1
...
def heartbeat(base_url: str, state: Dict[str, Any], last_batch_count: int, last_error: str | None) -> None:
    payload = {
        ...
        "metadata": {
            "platform": platform.platform(),
            "buffered": True,
            "agent_script_sha256": state.get("agent_script_sha256"),
            "package_manifest": state.get("package_manifest"),
            "startup_integrity_ok": bool(state.get("startup_integrity_ok", True)),
            "local_config_hash": state.get("config_hash"),
            "unexpected_restart_count": int(state.get("unexpected_restart_count", 0)),
...
def run_daemon(args: argparse.Namespace) -> int:
    ...
    previous_clean_shutdown = bool(state.get("last_shutdown_clean", True))
    if not previous_clean_shutdown:
        state["unexpected_restart_count"] = int(state.get("unexpected_restart_count", 0)) + 1
    state["last_start_at"] = now_iso()
    state["last_shutdown_clean"] = False
    state["agent_script_sha256"] = file_sha256(Path(__file__).resolve())
    manifest_status = validate_package_manifest(Path(args.package_manifest))
    state["package_manifest"] = manifest_status
    state["startup_integrity_ok"] = manifest_status.get("valid") is not False and bool(state.get("agent_script_sha256"))
    write_json(state_path, state)
...
def main() -> int:
    ...
    parser.add_argument("--package-manifest", default="MANIFEST.json", help="Optional package manifest with files/hash map for startup integrity checks")
