...
class TestConsumerGroupStability(unittest.TestCase):
    ...
    def test_initial_group_has_no_timestamp_suffix(self):
        ...
        self.assertEqual(group, "incident-builder-v1",
...
class TestConnectPgUsesBoundedPool(unittest.TestCase):
    ...
    def test_yields_none_when_pool_unavailable(self):
        with patch.object(ib.pg_pool, "get_pool", return_value=None):
            with ib.connect_pg() as conn:
    ...
    def test_borrows_from_pool_and_returns_it_on_exit(self):
        ...
        with patch.object(ib.pg_pool, "get_pool", return_value=fake_pool):
            with ib.connect_pg() as conn:
                self.assertIs(conn, fake_conn)
        fake_pool.connection.assert_called_once()

    def test_write_incidents_no_op_when_pool_unavailable(self):
        with patch.object(ib.pg_pool, "get_pool", return_value=None):
            result = ib.write_incidents([])
