...
func identityAlerts(actor string, group []Event) []Alert {
	...
	for _, ev := range group {
		...
		if ev.EventType == "login_failed" || ev.EventType == "mfa_failed" || ev.EventType == "mfa_failure_burst" || ev.Result == "failure" {
			failed = append(failed, ev)
			services[ev.EventSource] = true
		}
		if ev.EventType == "login_success" || ev.EventType == "suspicious_login" {
	...
	alerts := []Alert{}
	if len(failed) >= 5 || hasEvent(group, "identity", "mfa_failure_burst") {
...
func cloudSaasAlerts(actor string, group []Event) []Alert {
	...
	if len(downloads) >= 10 || hasEvent(group, "saas", "mass_download") {
		alerts = append(alerts, makeAlert("CLOUD_MASS_DOWNLOAD", actor, tail(downloads, 20), 0.76))
	}
	if len(keys) > 0 {
		alerts = append(alerts, makeAlert("CLOUD_NEW_ACCESS_KEY", actor, tail(keys, 5), 0.86))
	}
	if len(settings) > 0 {
		alerts = append(alerts, makeAlert("CLOUD_SECURITY_SETTING_MODIFIED", actor, tail(settings, 5), 0.86))
	}
	if len(admin) > 0 {
