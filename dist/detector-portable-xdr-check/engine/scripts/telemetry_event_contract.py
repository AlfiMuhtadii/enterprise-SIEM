...
SCHEMA_VERSION = 1
VALID_TYPES = {"endpoint", "network", "dns", "email", "identity", "cloud", "saas", "firewall", "proxy"}
...
def validate_event(event: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if telemetry_type in {"email", "identity", "cloud", "saas", "firewall", "proxy"}:
        if not isinstance(event.get("event_source"), str) or not str(event.get("event_source")).strip():
            errors.append("xdr.event_source is required for cross-domain telemetry")
        if event.get("risk_score") is not None:
            try:
                score = float(event.get("risk_score"))
                if score < 0 or score > 1:
                    errors.append("xdr.risk_score must be between 0 and 1")
            except (TypeError, ValueError):
                errors.append("xdr.risk_score must be numeric|null")
