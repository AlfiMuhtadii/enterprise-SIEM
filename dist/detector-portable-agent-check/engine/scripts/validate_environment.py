...
def validate(profile: str, env: Dict[str, str], allow_placeholders: bool = False) -> Tuple[List[str], List[str]]:
    ...
    if profile in {"staging", "production"}:
        ...
        if not env.get("SOC_AGENT_ENROLLMENT_TOKEN") or (not allow_placeholders and env.get("SOC_AGENT_ENROLLMENT_TOKEN") in {"change-me", "REPLACE_WITH_STRONG_AGENT_ENROLLMENT_TOKEN"}):
            errors.append("SOC_AGENT_ENROLLMENT_TOKEN must be set to a strong secret")
