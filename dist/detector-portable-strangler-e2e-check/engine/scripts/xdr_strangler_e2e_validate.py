...
def run_go_normalizer(rows: List[Dict[str, Any]], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for batch in chunks(rows, args.batch_size):
        ...
        if res.get("ok"):
            forwarded += int(body.get("forwarded") or 0)
            malformed += int(body.get("malformed") or 0)
        else:
            failed += len(batch)
    elapsed = max(time.perf_counter() - started, 0.001)
    return {"events": len(rows), "forwarded": forwarded, "malformed": malformed, "failed": failed, "elapsed_sec": elapsed, "throughput_eps": round(forwarded / elapsed, 2), "batch_latency_ms": percentiles(latencies)}
...
def main() -> int:
    ...
    report = {
        ...
        "comparison": {
            ...
            "normalization_go_eps": go_normalization["throughput_eps"],
    ...
    print(f"status={report['validation_status']} events={len(rows)} go_ingest_eps={go_ingestion['throughput_eps']} go_norm_eps={go_normalization['throughput_eps']} lag={normalized_lag}")
