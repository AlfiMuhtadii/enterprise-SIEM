...
{
    // Factor weights, level thresholds, and MAX_SCORE now live in
    // EntityRiskFactorScoring (CODE-STRUCT-DECOMPOSE) — kept here as
    // constant aliases so the 11 existing external references to
    // EntityRiskScoringService::WEIGHTS/LEVEL_THRESHOLDS/MAX_SCORE need no
    // changes, matching the ThreatHuntingService::SUPPORTED_DOMAINS
    // precedent (a class constant's value may reference another class's
    // constant; resolves correctly at runtime, not just parses).
    public const WEIGHTS = EntityRiskFactorScoring::WEIGHTS;
    public const LEVEL_THRESHOLDS = EntityRiskFactorScoring::LEVEL_THRESHOLDS;
    public const MAX_SCORE = EntityRiskFactorScoring::MAX_SCORE;
    ...
    {
        return EntityRiskFactorScoring::scoreToLevel($score);
    }

    public function aggregateScore(array $factors): float
    {
        return EntityRiskFactorScoring::aggregateScore($factors);
    }
    ...
    private function makeFactor(
        ...
        return EntityRiskFactorScoring::makeFactor($name, $value, $alertIds, $traceIds, $incidentIds, $advisoryOnly);
    }
}
