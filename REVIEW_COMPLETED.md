...
| **ENTERPRISE-068** | Container Resource Governance — `deploy.resources.limits` (memory + cpus) on 6 services in docker-compose.yml (dev) + docker-compose.prod.yml (prod); `xdr_container_resource_validate.py` 14 tests PASS | pending commit | — | 2026-06-29 |
| **ENTERPRISE-069** | PostgreSQL RLS Policy Scaffolding (advisory-only) — `scaffold_rls_policies` migration (DO $$, idempotent, pgsql-only, no enforcement); `TenantRlsStatusCommand` (read-only, advisory); `xdr_rls_scaffold_validate.py` 14 tests PASS; `RlsPolicyScaffoldingTest.php` 19 tests PASS | pending commit | — | 2026-06-29 |
| **ENTERPRISE-070** | Tenant Null Backfill Pre-Flight Validation — `TenantBackfillPreflightCommand` (6 CHK, advisory-only); GATE-08 `gateRlsPoliciesScaffolded()` in `TenantStrictModeReadinessService`; `xdr_tenant_backfill_preflight.py` 14 tests PASS; gate count 7→8 in test | pending commit | — | 2026-06-29 |
| **ENTERPRISE-071** | RAG Knowledge Base Operational Integration — `RagOperationalCheckCommand` (`ai:knowledge-check`, read-only); `xdr_rag_operational_validate.py` 14 tests PASS; `RagOperationalIntegrationTest.php` 12 tests PASS | pending commit | — | 2026-06-29 |
| **ENTERPRISE-072** | Shadow Domain Soak Pre-Flight — `DomainSoakHarnessService::getPreflightStatus()` (5 CHK, advisory); `ShadowSoakPreflightCommand` (`domain:soak-preflight`); `xdr_shadow_soak_preflight.py` 14 tests PASS; `ShadowSoakPreflightTest.php` 17 tests PASS | pending commit | — | 2026-06-29 |
| **ENTERPRISE-073** | Redpanda Multi-Node HA Template — `docker-compose.ha.yml` (3-broker, seed discovery, RPC addr, healthcheck, resource limits); `xdr_redpanda_ha_validate.py` 14 tests PASS; `--replication-factor` + `--replicas` flag in `xdr_topic_bootstrap.py` | pending commit | — | 2026-06-29 |
