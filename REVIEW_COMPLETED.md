...
| **GIT-RM-PYC** | Untracked 147 compiled `*.pyc`/`__pycache__` files + 3 Go `*.exe` service binaries from the git index (`git rm --cached`); added `*.exe` rules to `.gitignore` (`*.py[cod]`/`__pycache__/` already present). Removes perpetual `git status` noise; working files retained | (this batch) | — | 2026-06-30 |
