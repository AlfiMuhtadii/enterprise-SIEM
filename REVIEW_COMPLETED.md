...
| **PERF-AGENT-HEALTH-N1** | Agent health check N+1 → eager-load — `AgentHealthCheckCommand` eager-loads `agent_policies` (keyed map) + failure-breaching `endpoint_agents` (keyed map), and batches stale-agent offline status into one bulk UPDATE; staleness/alert semantics preserved; 3 tests (`PerfAgentN1Test`) | (this batch) | — | 2026-06-30 |
