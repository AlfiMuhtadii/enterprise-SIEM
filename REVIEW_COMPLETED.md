...
| **PERF-ALERT-TUNE** | Suppression apply N+1 → bulk — `SocTuningController::applyActiveSuppressions()` resolves matching alert ids once per rule, single bulk UPDATE + batched history insert (was 1 UPDATE + 1 INSERT per alert); retains 500-cap + unsuppressed-only semantics; 4 tests (`PerfAlertTuneTest`) | (this batch) | — | 2026-06-30 |
