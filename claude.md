...
php artisan test         → 1568 passed, zero failures
...
Threat hunting: replay-safe retrospective investigation across behavioral data, allowlisted bounded queries, advisory-only hunt records (append-only), no destructive operations. 45 supported domains.
...
Current: **1568 tests**, all green. Do NOT run parallel processes against the same PostgreSQL test database.
