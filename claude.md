...
php artisan migrate:fresh --force && php artisan test → 2024 passed, zero failures
...
Threat hunting: replay-safe retrospective investigation across behavioral data, allowlisted bounded queries, advisory-only hunt records (append-only), no destructive operations. 80 supported domains.
...
Current: **2024 tests**, all green. Do NOT run parallel processes against the same PostgreSQL test database.
Note: run `migrate:fresh` before the full suite when the DB has stale state; the `RefreshDatabase` trait's internal `migrate:fresh` can encounter conflicts with pre-existing data at 2000+ test scale.
