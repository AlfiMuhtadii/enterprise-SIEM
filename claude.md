...
php artisan test         → 2498 passed, zero failures (always prefix with migrate:fresh --force)
...
Threat hunting: replay-safe retrospective investigation across behavioral data, allowlisted bounded queries, advisory-only hunt records (append-only), no destructive operations. 115 supported domains.
...
Current: **2498 tests**, all green. Always prefix with `migrate:fresh --force` to avoid intermittent `QueryException` failures from stale schema state. Do NOT run parallel processes against the same PostgreSQL test database.
