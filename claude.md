...
php artisan test (in halves — see below) → 2093 passed, zero failures
...
Threat hunting: replay-safe retrospective investigation across behavioral data, allowlisted bounded queries, advisory-only hunt records (append-only), no destructive operations. 85 supported domains.
...
Current: **2093 tests**, all green. Do NOT run parallel processes against the same PostgreSQL test database.

Note: At 2093+ tests, the full sequential suite may have intermittent `QueryException` failures due to PostgreSQL connection pressure. All test CLASSES pass individually. For reliable validation at this scale, run in two halves:

```powershell
# First half (A-M)
php artisan test --filter="Active|Advanced|Baseline|Behavioral|Capacity|Compliance|CrossDomain|Demo|Detection|Distributed|Dns|Endpoint|Enterprise|Entity|Evidence|Example|Export|Fleet|Investigation|MultiTenant"
# Second half (N-Z)
php artisan test --filter="Network|Operations|Production|Profile|Release|Report|Resilience|Response|Scenario|Security|Sensor|Soc|Soar|SoakChaos|ThreatHunting|Trace|UEBA|Xdr"
```
