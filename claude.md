...
php artisan test         → 2629 passed, zero failures (always prefix with migrate:fresh --force)
...
Current: **2629 tests**, all green. Always prefix with `migrate:fresh --force` to avoid intermittent `QueryException` failures from stale schema state. Do NOT run parallel processes against the same PostgreSQL test database.
