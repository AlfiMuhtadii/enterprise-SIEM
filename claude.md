...
php artisan test         → 2236 passed, zero failures (always prefix with migrate:fresh --force)
python endpoint agent    → 186 tests, 0 failures
rule registry validator  → status=PASS  rules=93  checks=21/21
...
Threat hunting: replay-safe retrospective investigation across behavioral data, allowlisted bounded queries, advisory-only hunt records (append-only), no destructive operations. 95 supported domains.
...
Current: **2236 tests**, all green. Always prefix with `migrate:fresh --force` to avoid intermittent `QueryException` failures from stale schema state. Do NOT run parallel processes against the same PostgreSQL test database.
