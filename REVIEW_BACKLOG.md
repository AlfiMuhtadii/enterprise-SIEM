...
- **Remaining scope:** End-to-end verification against a real corporate IdP (Okta/Azure AD/ADFS) for both OIDC and SAML — needs a real external IdP to configure/test against, unavailable in this environment. `/sso/saml/metadata` is the concrete handoff point for a future real-IdP pass.
