...
| **TEST-NO-SCHEMA-DUMP** | [Test infra] No schema dump → 139 migrations run every fresh; mandated manual `migrate:fresh` doubles it (redundant with RefreshDatabase). Add `schema:dump --prune`; drop manual prefix (TEST-UNTRAITED audit cleared — all 7 untraited tests confirmed DB-free, see REVIEW_COMPLETED.md) | `database/schema/`, CLAUDE.md | Medium | Proposed |
