...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 29 ongoing - resilience validator mTLS isolation) | [Enterprise BLOCKER - reduced] Add opt-in fail-closed mTLS to the resilience validator's four internal service paths while keeping the internal client identity off the Laravel control-plane request. Remaining scope after this phase is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_resilience_validate.py`, `tests/scripts/test_xdr_resilience_validate.py`, `docs/validation/VALIDATION_BASELINES.md` | High | Ongoing |
