...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-38 done - polyglot smoke supports isolated mTLS) | [Enterprise BLOCKER - reduced] Polyglot runtime validation now uses opt-in fail-closed mTLS for six first-party service health, ingestion, and metrics paths while retaining separate Redpanda, ClickHouse, OpenSearch, and Qdrant trust contexts. Remaining scope is other noncanonical scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_polyglot_microservices_validate.py`, `tests/scripts/test_xdr_polyglot_microservices_validate.py`, `docs/architecture/polyglot-microservices-xdr.md` | High | Proposed (reduced) |
