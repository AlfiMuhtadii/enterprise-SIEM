...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-15 done — service TLS scaffolding, Pandaproxy/OpenSearch TLS, native Kafka TLS, PostgreSQL mTLS, ClickHouse HTTPS, Qdrant TLS, and Laravel-to-AI-RAG mTLS are implemented) | [Enterprise BLOCKER — reduced] Phase 15 enables fail-closed production mutual TLS for Laravel-to-AI-RAG calls, with CA/hostname verification, client certificates, private keys restricted to root/group `44444`, completion-gated certificate initialization, and a semantic CI gate. Live Docker proof used the real Uvicorn service: valid Python and Laravel clients returned 200 while a client without a certificate was disconnected. Remaining scope includes the other application microservice HTTP endpoints plus Ollama, Tempo/OTLP, and MinIO where enabled. | `docker-compose*.yml`, `app/Services/InternalMtlsHttpClient.php`, `app/Support/AiRagServiceProvider.php`, `scripts/xdr_ai_rag_mtls_compose_validate.py` | High | Proposed (reduced; AI-RAG mTLS phase completed by Codex, 2026-08-30) |
