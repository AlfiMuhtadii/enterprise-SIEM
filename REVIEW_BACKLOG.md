...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-36 done - correlation soak supports mTLS) | [Enterprise BLOCKER - reduced] Correlation soak now uses opt-in fail-closed mTLS for POST and metrics requests while preserving retry, PHP cutover-status, duration, and report semantics. Remaining scope is other noncanonical scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_correlation_soak.py`, `tests/scripts/test_xdr_correlation_soak.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Proposed (reduced) |
