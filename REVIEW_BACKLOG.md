...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-24 done; phase 25 live-soak mTLS ongoing) | [Enterprise BLOCKER - reduced] Phase 25 is adding opt-in, fail-closed mTLS to both live-soak preflight and persistent ingestion connections while preserving plaintext local defaults and injected offline tests. Remaining scope is other noncanonical demo/validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_live_soak_validate.py`, `tests/xdr_topic_bootstrap/test_xdr_live_soak_validate.py` | High | Ongoing (Codex, 2026-08-30) |
