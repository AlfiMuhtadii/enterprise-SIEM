...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 39 ongoing - live-pipeline all-service mTLS) | [Enterprise BLOCKER - reduced] Extend the live-pipeline readiness validator from ingestion-only mTLS to an opt-in all-first-party mode covering health, movement metrics, and auth-posture metrics while preserving Redpanda trust isolation and ingestion-only CLI compatibility. | `scripts/validate_live_xdr_pipeline.py`, `tests/demo_causal_verify/test_validate_live_pipeline_mtls.py`, `docs/guides/DEMO_CAUSAL_PROOF.md` | High | Ongoing |
