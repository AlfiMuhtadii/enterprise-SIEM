...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-23 done; phase 24 production ingestion cutover ongoing) | [Enterprise BLOCKER - reduced] Phase 24 is coordinating production gateway server mTLS, a certificate-authenticated gateway healthcheck, client-only mTLS for all five connectors, HTTPS Laravel/scenario URLs, private-key mounts/groups, and removal of connector metrics host ports. Remaining scope after this cutover is noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `docker-compose.prod.yml`, `services/ingestion-gateway/Dockerfile`, production mTLS validator/runbook | High | Ongoing (Codex, 2026-08-30) |
