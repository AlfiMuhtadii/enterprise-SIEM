...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 35 ongoing - correlation benchmark mTLS) | [Enterprise BLOCKER - reduced] Phase 35 is adding opt-in fail-closed mTLS to correlation benchmark health, correlate, and metrics requests without changing benchmark or fallback semantics. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_correlation_shadow_benchmark.py`, `tests/scripts/test_xdr_correlation_shadow_benchmark.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Ongoing |
