...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 36 ongoing - correlation soak mTLS) | [Enterprise BLOCKER - reduced] Phase 36 is adding opt-in fail-closed mTLS to correlation soak POST and metrics requests while preserving retry, PHP cutover-status, duration, and report semantics. Remaining scope is other noncanonical scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_correlation_soak.py`, `tests/scripts/test_xdr_correlation_soak.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Ongoing |
