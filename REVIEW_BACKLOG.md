...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-39 done - live-pipeline all-service mTLS) | [Enterprise BLOCKER - reduced] Live-pipeline readiness validation now offers an opt-in all-first-party mTLS mode covering health, movement metrics, and auth-posture metrics while preserving Redpanda trust isolation and ingestion-only CLI compatibility. Remaining scope is other noncanonical scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/validate_live_xdr_pipeline.py`, `tests/demo_causal_verify/test_validate_live_pipeline_mtls.py`, `docs/guides/DEMO_CAUSAL_PROOF.md` | High | Proposed (reduced) |
