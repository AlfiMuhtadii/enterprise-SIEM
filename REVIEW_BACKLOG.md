...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-24 done - production canonical ingestion cut over to mTLS) | [Enterprise BLOCKER - reduced] Production now requires mTLS on ingestion-gateway, runs a CA/hostname/client-certificate healthcheck, routes all five bundled Go connectors through client-only mTLS, and points Laravel service-health/scenario clients to HTTPS. Connectors wait for a healthy gateway and no connector metrics ports remain host-exposed. Endpoint-agent support remains opt-in with per-endpoint identity provisioning. Remaining scope is mTLS support for noncanonical validation/demo/soak scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `docker-compose.prod.yml`, `services/ingestion-gateway/Dockerfile`, `scripts/xdr_ingestion_mtls_compose_validate.py`, `docs/operations/INGESTION_GATEWAY_MTLS_CUTOVER.md` | High | Proposed (reduced) |
