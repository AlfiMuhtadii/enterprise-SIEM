...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 45 ongoing - shared Ollama client TLS verification) | [Enterprise BLOCKER - reduced] Add one URL-validating, private-CA-aware Laravel HTTP client for both Ollama generation and embedding calls while preserving local plaintext behavior. Server-side TLS termination remains a separate deployment prerequisite. | `app/Services/OllamaHttpClient.php`, `app/Support/RemoteLlmProvider.php`, `app/Support/SocKnowledgeRetriever.php`, `config/soc.php`, `.env.example`, `.env.production.example`, `tests/Feature/OllamaTlsClientTest.php`, `docs/guides/README_SOC_LLM_RAG_GUARDRAILS.md` | High | Ongoing |
