...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 34 ongoing - threat-intel correlation validation mTLS) | [Enterprise BLOCKER - reduced] Phase 34 is adding opt-in fail-closed mTLS to the threat-intel validator's actual correlation-worker health and correlate requests while preserving local IOC validation. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_threat_intel_validate.py`, `tests/scripts/test_xdr_threat_intel_validate.py`, `docs/operations/threat-intel-validation.md` | High | Ongoing |
