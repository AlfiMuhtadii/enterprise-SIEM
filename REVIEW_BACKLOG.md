...
| **ENT-SDLC-NO-SUPPLYCHAIN** (base-image digest pinning, SBOM, dependency scan, container scan, signed release publication, and deployment verifier implemented) | [Enterprise SDLC - reduced] `xdr_release_verify.py` now validates the complete 13-image manifest, requires independently approved repository/tag/commit inputs, and verifies every `image@sha256:digest` with exact Cosign identity, GitHub Actions OIDC issuer, workflow SHA, repository, and tag-ref certificate claims. It fails closed on malformed/tampered evidence, missing Cosign, invalid signatures, and timeouts, while emitting machine-readable deployment evidence. Release publication is restricted to the exact selected tag so branch identities cannot sign deployable artifacts. Remaining scope is a real signed-release/live deployment run and a justified vulnerability blocking policy; no staging target currently exists. | `.github/workflows/release.yml`, `scripts/xdr_release_{manifest,verify}.py`, `docs/operations/IMMUTABLE_RELEASE_VERIFICATION.md`, deployment policy | Medium | Proposed (reduced; verifier implemented by Codex, 2026-08-25) |
