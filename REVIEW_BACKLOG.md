...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 44 ongoing - JSONL tail producer Pandaproxy TLS) | [Enterprise BLOCKER - reduced] Add private-CA/hostname verification to the long-running JSONL tail producer, validate transport before state-file mutation, and preserve local plaintext behavior. | `scripts/stream_producer_jsonl.py`, `tests/scripts/test_stream_producer_jsonl.py`, `docs/operations/PANDAPROXY_BOOTSTRAP_TLS.md` | High | Ongoing |
