...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 41 ongoing - Pandaproxy bootstrap TLS verification) | [Enterprise BLOCKER - reduced] Add explicit private-CA verification to the canonical Redpanda topic bootstrap path without presenting the internal service client identity or changing local plaintext defaults. | `scripts/xdr_topic_bootstrap.py`, `tests/xdr_topic_bootstrap/test_xdr_topic_bootstrap.py`, `docs/operations/PANDAPROXY_BOOTSTRAP_TLS.md` | High | Ongoing |
