...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-13 done — service TLS scaffolding, Pandaproxy/OpenSearch TLS, native Kafka TLS, PostgreSQL mTLS, and ClickHouse HTTPS are implemented) | [Enterprise BLOCKER — reduced] Phase 13 adds opt-in local and fail-closed production ClickHouse HTTPS, removes the plaintext listener whenever TLS is enabled, and gives every Laravel/Python ClickHouse client CA-verified transport. A semantic CI gate and live Docker proof cover service-DNS hostname verification, plaintext refusal, and untrusted-CA rejection. Remaining scope is Qdrant and any other internal infrastructure clients still using plaintext transport. | `docker-compose*.yml`, `.env*.example`, `infra/clickhouse/https.xml`, `scripts/xdr_{clickhouse_tls_compose_validate,infra_clients}.py`, `app/Services/ClickHouse*.php` | High | Proposed (reduced; ClickHouse HTTPS phase completed by Codex, 2026-08-30) |
