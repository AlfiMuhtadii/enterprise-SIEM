...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-10 done — service TLS scaffolding, Pandaproxy/OpenSearch TLS infrastructure, downstream Python CA verification, and native Kafka TLS clients are implemented; Postgres full mTLS plus remaining Go/PHP wiring remain) | [Enterprise BLOCKER — reduced] Phase 9 added destination-scoped CA verification for every Pandaproxy REST call in `alert-writer-service` and `incident-builder-service`, plus verified-CA OpenSearch writes in alert-writer. Phase 10 added opt-in, fail-closed native Kafka TLS to all three Go pipeline services: CA validation and hostname verification are mandatory, optional client mTLS requires a complete cert/key pair, plaintext constructors remain compatible, and a canonical independently tested package plus CI drift gate keeps all copies identical. Real generated-CA TLS producer/consumer handshakes passed in every service and all four Go suites passed the Linux race detector. Remaining scope is a Redpanda native Kafka TLS listener plus Compose cert mounts, PostgreSQL server/client mTLS, Laravel/PHP CA wiring, and other plaintext infrastructure clients. | `services/{ingestion-gateway,normalizer-worker,correlation-worker}`, `tools/shared-go/kafkanative`, `services/{alert-writer-service,incident-builder-service}/main.py`, `docker-compose.yml`, `.env*.example`, `config/database.php`, `app/Services/InternalAuthService.php` | High | Proposed (reduced; native Kafka TLS client phase completed by Codex, 2026-08-27) |
