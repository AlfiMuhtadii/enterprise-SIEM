...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-14 done — service TLS scaffolding, Pandaproxy/OpenSearch TLS, native Kafka TLS, PostgreSQL mTLS, ClickHouse HTTPS, and Qdrant TLS are implemented) | [Enterprise BLOCKER — reduced] Phase 14 makes Qdrant REST and gRPC TLS opt-in locally and mandatory/fail-closed in production. Laravel knowledge retrieval/upsert/storage validation, Python infrastructure clients, and the polyglot smoke validator now use destination-scoped CA verification. A semantic CI gate and live Qdrant 1.12.1 Docker proof cover service-DNS hostname verification, plaintext rejection, and untrusted-CA rejection. Remaining scope includes Ollama, Tempo/OTLP, MinIO where enabled, and application microservice HTTP endpoints that still use plaintext transport. | `docker-compose*.yml`, `.env*.example`, `scripts/xdr_{qdrant_tls_compose_validate,infra_clients,polyglot_microservices_validate}.py`, `app/Services/QdrantHttpClient.php`, `app/Support/SocKnowledgeRetriever.php` | High | Proposed (reduced; Qdrant TLS phase completed by Codex, 2026-08-30) |
