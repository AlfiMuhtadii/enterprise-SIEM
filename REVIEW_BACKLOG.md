...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-45 done - shared Ollama client TLS verification) | [Enterprise BLOCKER - reduced] Ollama generation and embedding now share one URL-validating client with system/private-CA hostname verification, fail-closed CA configuration, and explicit diagnostic-only insecure mode. Local HTTP remains compatible. Remaining scope: deploy server-side Ollama TLS termination, secure Tempo/OTLP, and audit other noncanonical internal clients. | `app/Services/OllamaHttpClient.php`, `app/Support/RemoteLlmProvider.php`, `app/Support/SocKnowledgeRetriever.php`, `config/soc.php`, `.env.example`, `.env.production.example`, `tests/Feature/OllamaTlsClientTest.php`, `docs/guides/README_SOC_LLM_RAG_GUARDRAILS.md` | High | Proposed (reduced) |
