...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-31 done - event-flow service paths support isolated mTLS) | [Enterprise BLOCKER - reduced] The event-flow validator now uses opt-in fail-closed mTLS for alert-writer and incident-builder while keeping the internal client identity off Redpanda REST. Local plaintext defaults remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_event_flow_resilience_validate.py`, `tests/scripts/test_xdr_event_flow_resilience_validate.py`, `docs/validation/VALIDATION_BASELINES.md` | High | Proposed (reduced) |
