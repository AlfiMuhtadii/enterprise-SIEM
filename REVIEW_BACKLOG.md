...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-25 done - live-soak supports mTLS) | [Enterprise BLOCKER - reduced] The live-soak validator now supports opt-in fail-closed mTLS for both gateway health preflight and its persistent ingestion connection, with CA/hostname verification and client identity loading. Local plaintext defaults and offline injection contracts remain unchanged. Remaining scope is other noncanonical demo/validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_live_soak_validate.py`, `tests/xdr_topic_bootstrap/test_xdr_live_soak_validate.py`, `docs/operations/LIVE_SOAK_VALIDATION.md` | High | Proposed (reduced) |
