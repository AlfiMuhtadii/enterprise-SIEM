...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-22 done; phase 23 connector rollout controls ongoing) | [Enterprise BLOCKER - reduced] Phase 23 is separating mTLS server and ingestion-client enablement across all five Go connectors so production can roll out gateway transport independently from connector health/metrics listeners. Defaults remain backward-compatible: the client inherits the server flag unless explicitly overridden. Remaining scope is production Compose/certificate/health orchestration and gateway cutover, plus noncanonical validation scripts, Ollama, Tempo/OTLP, and MinIO where enabled. | `services/log-connector-*/{main.go,main_test.go}` | High | Ongoing (Codex, 2026-08-30) |
