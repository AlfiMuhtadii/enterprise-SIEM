...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 38 ongoing - polyglot smoke mTLS isolation) | [Enterprise BLOCKER - reduced] Add opt-in fail-closed mTLS to the polyglot runtime validator's six first-party service health, ingestion, and metrics paths while keeping Redpanda, ClickHouse, OpenSearch, and Qdrant on separate trust contexts. | `scripts/xdr_polyglot_microservices_validate.py`, `tests/scripts/test_xdr_polyglot_microservices_validate.py`, `docs/architecture/polyglot-microservices-xdr.md` | High | Ongoing |
