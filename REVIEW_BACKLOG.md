...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-12 done — service TLS scaffolding, Pandaproxy/OpenSearch TLS, native Kafka TLS, and PostgreSQL mTLS are implemented) | [Enterprise BLOCKER — reduced] Phase 12 adds opt-in local and fail-closed production PostgreSQL TLS, CA-verified client certificates, `verify-full` hostname validation for all seven Compose database clients, scoped leaf-certificate EKUs, non-root private-key access, a 54-check semantic CI gate, and real TLS 1.3 live proof. Plaintext, missing-client-certificate, and untrusted-CA probes were all rejected. Remaining scope is ClickHouse/Qdrant and any other internal infrastructure clients still using plaintext transport. | `docker-compose*.yml`, `.env*.example`, `scripts/xdr_*_tls_*`, `services/{ingestion-gateway,normalizer-worker,correlation-worker,alert-writer-service,incident-builder-service}`, `tools/shared-{go,python}`, `config/database.php`, `app/Services/InternalAuthService.php` | High | Proposed (reduced; PostgreSQL mTLS phase completed by Codex, 2026-08-28) |
...
## Completed Task: TENANT-FORENSIC-ISOLATION — Scope Forensic Requests and Build Scoped Artifacts

- **Work status:** Completed by existing commit `93b85e8`; duplicate backlog candidate verified and closed by Codex on 2026-08-28. Migration/index, tenant-boundary registration, cross-tenant agent/job rejection, and strict artifact scoping for alerts/agent metadata are covered by six focused feature tests. Full remote Laravel/PostgreSQL CI run `33168183813` passed; local database-backed PHP execution remains unavailable because host PHP lacks `pdo_pgsql`.
- **Priority:** High
- **Component:** `app/Http/Controllers/SocForensicController.php`, database schemas
- **Finding:** `SocForensicController` inserts forensic collection jobs into `forensic_collection_jobs` without storing a `tenant_id` or verifying that the targeted EDR agent belongs to the requesting analyst's tenant. Additionally, when building the forensic JSON/ZIP artifact, it queries `security_alerts` and `endpoint_agents` globally, allowing Tenant A's analyst to leak alerts and metadata belonging to Tenant B.
- **Proposed Fix:** Add `tenant_id` to the `forensic_collection_jobs` table, register it under `ISOLATED_TABLES` in `TenantBoundaryService`, scope all creation and fetch operations by the resolved tenant context, and strictly filter the nested `security_alerts` and `endpoint_agents` queries by `tenant_id`.
- **Validation Gate:** A tenant-scoped analyst trying to trigger a forensic job for another tenant's agent is rejected with a 403, and compiled artifacts do not include other tenants' alerts or agent metadata.

## Completed Task: TENANT-HUNT-CORRELATION-ISOLATION — Harden Threat Hunt Correlation Scoping

- **Work status:** Completed by existing commit `93b85e8`; duplicate backlog candidate verified and closed by Codex on 2026-08-28. The implementation persists tenant IDs, scopes saved/run histories to the active tenant plus the documented legacy-null compatibility path, and strictly tenant-scopes correlated alerts. Full remote Laravel/PostgreSQL CI run `33168183813` passed. The focused local test could not start because host PHP lacks `pdo_pgsql`; all seven failures were environment-level `could not find driver` errors with zero assertions executed.
- **Priority:** High
- **Component:** `app/Http/Controllers/SocHuntController.php`
