...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-40 done - MinIO cold-tier TLS) | [Enterprise BLOCKER - reduced] The optional production MinIO cold tier now serves CA-verified HTTPS without host ports, while Laravel's S3 client verifies the internal CA and local plaintext defaults remain compatible. Remaining scope is other noncanonical scripts plus Ollama and Tempo/OTLP where enabled. | `docker-compose.yml`, `docker-compose.prod.yml`, `config/filesystems.php`, `scripts/xdr_minio_tls_compose_validate.py`, `docs/operations/MINIO_COLD_TIER_TLS.md` | High | Proposed (reduced) |
