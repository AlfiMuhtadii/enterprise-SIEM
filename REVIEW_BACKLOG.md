...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-21 done - O365 internal/external transports isolated) | [Enterprise BLOCKER - reduced] Phase 21 removes the O365 connector's shared HTTP transport: ingestion now has a dedicated client for private-CA mTLS, while Microsoft OAuth and Activity API calls share a separate public-PKI client that cannot inherit the internal certificate or root pool. Audit confirmed all five Go connectors already contain code-level mTLS builders and ingestion transport wiring. Remaining scope is production Compose/certificate/health orchestration for those connectors and Laravel scenario clients before enabling the ingestion-gateway mTLS server, plus Ollama, Tempo/OTLP, and MinIO where enabled. | `services/log-connector-o365/{main.go,main_test.go,README.md}` | High | Proposed (reduced) |
