...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 28 ongoing - strangler E2E first-party mTLS) | [Enterprise BLOCKER - reduced] Add opt-in fail-closed mTLS to the strangler E2E validator's gateway, normalizer, and AI HTTP paths while keeping the client identity isolated from Redpanda. Remaining scope after this phase is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_strangler_e2e_validate.py`, `tests/scripts/test_xdr_strangler_e2e_validate.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Ongoing |
