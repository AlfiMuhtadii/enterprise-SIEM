...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 43 ongoing - stream-bus Pandaproxy TLS and failure status) | [Enterprise BLOCKER - reduced] Add private-CA/hostname verification to the Redpanda backend of the stream-bus utility and stop reporting exit 0 when one or more publishes were diverted to DLQ. | `scripts/xdr_stream_bus.py`, `tests/scripts/test_xdr_stream_bus.py`, `docs/operations/PANDAPROXY_BOOTSTRAP_TLS.md` | High | Ongoing |
