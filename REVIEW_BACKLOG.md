...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-28 done - strangler E2E supports first-party mTLS) | [Enterprise BLOCKER - reduced] The strangler E2E validator now uses opt-in fail-closed mTLS for gateway, normalizer, and AI HTTP paths while keeping the client identity isolated from Redpanda. Local plaintext defaults remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_strangler_e2e_validate.py`, `tests/scripts/test_xdr_strangler_e2e_validate.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Proposed (reduced) |
