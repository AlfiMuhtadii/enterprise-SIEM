...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-34 done - threat-intel correlation checks support mTLS) | [Enterprise BLOCKER - reduced] Threat-intel correlation service checks now use opt-in fail-closed mTLS for their actual health and correlate requests, while local IOC validation remains certificate-independent. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_threat_intel_validate.py`, `tests/scripts/test_xdr_threat_intel_validate.py`, `docs/operations/threat-intel-validation.md` | High | Proposed (reduced) |
