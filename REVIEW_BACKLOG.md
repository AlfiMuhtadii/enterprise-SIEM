...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-9 done — service TLS scaffolding, Pandaproxy/OpenSearch TLS infrastructure, and downstream Python HTTP client CA verification are live-verified; Postgres full mTLS plus remaining Go/PHP client wiring remain) | [Enterprise BLOCKER — reduced] Phase 9 added destination-scoped CA verification for every Pandaproxy REST call in `alert-writer-service` and `incident-builder-service`, plus verified-CA OpenSearch writes in alert-writer. Defaults remain plaintext-compatible; HTTPS is opt-in through internal URLs, CA paths, and explicit verify toggles. Compose mounts the CA read-only, keeps host and container Pandaproxy URLs separate, and the cert generator now covers the `opensearch-tls` service hostname. Live proof: alert-writer received HTTP 200 from real `https://redpanda:8083` with CA verification and indexed one document through real OpenSearch TLS; the test document was deleted and the opt-in OpenSearch TLS instance stopped afterward. Remaining scope is PostgreSQL server/client mTLS, native Kafka TLS, Laravel/PHP CA wiring, and any Go client still using plaintext infrastructure endpoints | `services/{alert-writer-service,incident-builder-service}/main.py`, `docker-compose.yml`, `.env*.example`, `scripts/xdr_generate_internal_mtls_certs.py`, `config/database.php`, `app/Services/InternalAuthService.php` | High | Proposed (reduced) |
...
- **Component:** `services/endpoint-agent/agent.py`, `deploy/agent/`, `scripts/build_agent_package.py`, `scripts/export_portable_detector.py`, 4 docs/guides files
...
- **Remaining scope:** Signed artifacts + manifest/hash verification at install time, and validation on clean Windows/Linux VMs without a Python runtime — this environment has neither a code-signing setup nor spare VMs, so that part stays open.
