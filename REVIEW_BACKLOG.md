...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 40 ongoing - MinIO cold-tier TLS) | [Enterprise BLOCKER - reduced] Enable CA-verified TLS for the optional production MinIO cold tier, seed correctly named server material, remove production host ports, and make Laravel's S3 client verify the internal CA without changing local plaintext defaults. | `docker-compose.yml`, `docker-compose.prod.yml`, `config/filesystems.php`, `scripts/xdr_minio_tls_compose_validate.py`, `docs/operations/MINIO_COLD_TIER_TLS.md` | High | Ongoing |
