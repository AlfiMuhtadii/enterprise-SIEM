...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 27 ongoing - causal verifier ingestion mTLS) | [Enterprise BLOCKER - reduced] Extend the causal verifier and its readiness subprocess with opt-in fail-closed ingestion mTLS while keeping the client identity isolated from non-ingestion HTTP checks. Remaining scope after this phase is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/demo_causal_verify.py`, `scripts/validate_live_xdr_pipeline.py`, `tests/demo_causal_verify/`, `docs/guides/DEMO_CAUSAL_PROOF.md` | High | Ongoing |
