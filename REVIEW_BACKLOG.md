...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-29 done - resilience validator supports isolated mTLS) | [Enterprise BLOCKER - reduced] The resilience validator now uses opt-in fail-closed mTLS for its four internal service paths while keeping the internal client identity off the Laravel control-plane request. Local plaintext defaults remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_resilience_validate.py`, `tests/scripts/test_xdr_resilience_validate.py`, `docs/validation/VALIDATION_BASELINES.md` | High | Proposed (reduced) |
