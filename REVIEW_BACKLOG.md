...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-26 done - demo-feed supports mTLS) | [Enterprise BLOCKER - reduced] The demo feed now supports opt-in fail-closed mTLS for both gateway health preflight and event ingestion, with CA/hostname verification and client identity loading. Local plaintext defaults and injected offline test contracts remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/demo_feed.py`, `tests/demo_feed/test_demo_feed.py`, `docs/guides/DEMO_CAUSAL_PROOF.md` | High | Proposed (reduced) |
