...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-30 done - fault-injection service paths support isolated mTLS) | [Enterprise BLOCKER - reduced] The fault-injection tool now uses opt-in fail-closed mTLS for its ingestion-gateway, normalizer, and alert-writer paths while keeping the internal client identity off Laravel. Local plaintext defaults remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_fault_injection.py`, `tests/scripts/test_xdr_fault_injection.py`, `docs/validation/VALIDATION_BASELINES.md` | High | Proposed (reduced) |
