...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-37 done - staged-cutover validation supports mTLS) | [Enterprise BLOCKER - reduced] Staged-cutover validation now propagates opt-in fail-closed mTLS to normal shadow/active benchmark subprocesses while isolating intentional fallback and PHP rollback probes from the client identity. Remaining scope is other noncanonical scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_staged_cutover_validate.py`, `tests/scripts/test_xdr_staged_cutover_validate.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Proposed (reduced) |
