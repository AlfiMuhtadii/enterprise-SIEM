...
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 1 done — ingestion-gateway cert tooling + server/client mTLS, off by default; other hops remain) | [Enterprise BLOCKER] Cert generation (`scripts/xdr_generate_internal_mtls_certs.py`) and a reusable `internal/mtls` Go package (`ServerConfig`/`ClientConfig`, requires+verifies client certs) now exist and are wired into `ingestion-gateway`'s own listener and outbound `httpClient`, gated behind `XDR_INTERNAL_MTLS_ENABLED` (default `false` — zero behavior change until an operator opts in). Verified with a real end-to-end handshake smoke test (binary built and run, real generated certs, no-cert request rejected / valid-cert request succeeds) in addition to unit tests. Still missing: the other 9 services (normalizer-worker, correlation-worker, alert-writer-service, incident-builder-service, ai-rag-service, 4 log-connector-*), Pandaproxy/OpenSearch plaintext HTTP, Postgres sslmode, and the docker-compose network wiring to actually run mTLS between containers — this phase proves the mechanism on one representative service, it does not cover "any internal hop" yet | `services/ingestion-gateway/internal/mtls/`, `scripts/xdr_generate_internal_mtls_certs.py`, `app/Services/InternalAuthService.php`, DB DSNs | High | Proposed (reduced) |
