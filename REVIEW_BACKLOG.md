...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-27 done - causal verifier supports ingestion mTLS) | [Enterprise BLOCKER - reduced] The causal verifier now propagates opt-in fail-closed mTLS through gateway readiness and demo ingestion while isolating the client identity from non-ingestion checks. Local plaintext defaults remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/demo_causal_verify.py`, `scripts/validate_live_xdr_pipeline.py`, `tests/demo_causal_verify/`, `docs/guides/DEMO_CAUSAL_PROOF.md` | High | Proposed (reduced) |
