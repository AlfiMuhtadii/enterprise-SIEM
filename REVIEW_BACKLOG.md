...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-23 done - connector server/client rollout controls separated) | [Enterprise BLOCKER - reduced] Phase 23 gives all five Go connectors independent mTLS controls for their health/metrics server and ingestion client. The client flag inherits the server flag by default for compatibility, but can be explicitly enabled alone for an ingestion-only cutover. Missing identity material remains fail-closed. Remaining scope is production Compose certificate/health orchestration and the coordinated ingestion-gateway server cutover, plus noncanonical validation scripts, Ollama, Tempo/OTLP, and MinIO where enabled. | `services/log-connector-*/{main.go,mtls_modes_test.go}`, `docs/operations/CONNECTOR_INGESTION_MTLS.md` | High | Proposed (reduced) |
