...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-19 done - infrastructure transport plus all five internal Go/Python HTTP services are protected) | [Enterprise BLOCKER - reduced] Phase 19 adds production mTLS to incident-builder-service, routes Laravel service health and resilience checks through internal HTTPS, keeps private keys root/group `44444`, gates startup on certificate initialization, and removes the host port. The image's own CA-verifying, client-certificate healthcheck remained healthy under the real Uvicorn listener. Live rebuilt-service proof returned 200 to CA/hostname-verified Python and Laravel clients and rejected a client without a certificate. Remaining scope includes the ingestion-gateway inbound API and its endpoint/connector clients, plus Ollama, Tempo/OTLP, and MinIO where enabled. | `docker-compose*.yml`, `app/Services/ResilienceValidationService.php`, `scripts/xdr_incident_builder_mtls_compose_validate.py` | High | Ongoing (Codex, 2026-08-30) |
