...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-42 done - demo-alert Pandaproxy TLS) | [Enterprise BLOCKER - reduced] The topic bootstrap and deterministic demo-alert publisher now verify the private CA and hostname for the TLS Pandaproxy listener, while local plaintext defaults remain compatible. Remaining scope is Ollama, Tempo/OTLP, and other noncanonical clients where enabled. | `scripts/xdr_topic_bootstrap.py`, `scripts/xdr_send_demo_alert.py`, `docs/operations/PANDAPROXY_BOOTSTRAP_TLS.md` | High | Proposed (reduced) |
