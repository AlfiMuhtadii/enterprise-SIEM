...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 26 ongoing - demo-feed mTLS transport) | [Enterprise BLOCKER - reduced] Add opt-in fail-closed mTLS to the demo feed's ingestion health preflight and event POST path while preserving local plaintext defaults and injected offline test contracts. Remaining scope after this phase is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/demo_feed.py`, `tests/demo_feed/test_demo_feed.py`, `docs/guides/DEMO_CAUSAL_PROOF.md` | High | Ongoing |
