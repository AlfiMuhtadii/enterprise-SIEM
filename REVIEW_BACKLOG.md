...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 32 ongoing - endpoint-normalization validator mTLS) | [Enterprise BLOCKER - reduced] Phase 32 is adding opt-in fail-closed mTLS to endpoint-normalization service-mode health and normalize requests while preserving offline local validation. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_endpoint_normalization_validate.py`, `tests/scripts/test_xdr_endpoint_normalization_validate.py`, `docs/operations/endpoint-telemetry-validation.md` | High | Ongoing |
