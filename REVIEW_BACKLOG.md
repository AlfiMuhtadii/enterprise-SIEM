...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-22 done - canonical scenario ingestion supports mTLS) | [Enterprise BLOCKER - reduced] Phase 22 routes real-mode Laravel scenario ingestion through the existing fail-closed `InternalMtlsHttpClient`. Plain HTTP remains backward-compatible for local runs; HTTPS now requires the configured CA and client identity and retains hostname verification. Remaining scope is production Compose/certificate/health orchestration for all five Go connectors before enabling the ingestion-gateway mTLS server, plus noncanonical validation scripts, Ollama, Tempo/OTLP, and MinIO where enabled. | `app/Jobs/ExecuteScenarioRunJob.php`, `tests/Feature/ScenarioRunnerTest.php` | High | Proposed (reduced) |
