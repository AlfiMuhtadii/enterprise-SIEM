...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-44 done - JSONL tail producer Pandaproxy TLS) | [Enterprise BLOCKER - reduced] Long-running JSONL tail publishing now supports private-CA and hostname verification, rejects CA use on plaintext URLs before state-file mutation, and preserves the local HTTP default. Remaining scope: Ollama, Tempo/OTLP, and other noncanonical internal clients. | `scripts/stream_producer_jsonl.py`, `tests/scripts/test_stream_producer_jsonl.py`, `docs/operations/PANDAPROXY_BOOTSTRAP_TLS.md` | High | Proposed (reduced) |
