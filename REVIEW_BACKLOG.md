...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-35 done - correlation benchmark supports mTLS) | [Enterprise BLOCKER - reduced] The correlation benchmark now uses opt-in fail-closed mTLS for health, correlate, and metrics requests without changing benchmark or fallback semantics. Remaining scope is correlation soak and other noncanonical scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_correlation_shadow_benchmark.py`, `tests/scripts/test_xdr_correlation_shadow_benchmark.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Proposed (reduced) |
