...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-33 done - endpoint shadow-correlation service mode supports mTLS) | [Enterprise BLOCKER - reduced] Endpoint shadow-correlation service mode now uses opt-in fail-closed mTLS for health and correlate requests, while offline simulation remains certificate-independent. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_endpoint_shadow_correlation_validate.py`, `tests/scripts/test_xdr_endpoint_shadow_correlation_validate.py`, `docs/operations/endpoint-shadow-validation.md` | High | Proposed (reduced) |
