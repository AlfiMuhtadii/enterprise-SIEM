...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-20 done; phase 21 O365 connector transport isolation ongoing) | [Enterprise BLOCKER - reduced] Phase 21 is separating the O365 connector's internal ingestion transport from its external Microsoft OAuth/Activity API transport before production mTLS wiring. A shared client would replace public roots with the internal CA and present the internal client identity to third-party endpoints. Remaining scope includes production mTLS orchestration for all connector/scenario clients, then ingestion-gateway server cutover, plus Ollama, Tempo/OTLP, and MinIO where enabled. | `services/log-connector-o365/main.go`, `services/log-connector-o365/main_test.go` | High | Ongoing (Codex, 2026-08-30) |
