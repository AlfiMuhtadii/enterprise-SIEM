...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-16 done - service TLS scaffolding, Pandaproxy/OpenSearch TLS, native Kafka TLS, PostgreSQL mTLS, ClickHouse HTTPS, Qdrant TLS, Laravel-to-AI-RAG mTLS, and Laravel-to-correlation-worker mTLS are implemented) | [Enterprise BLOCKER - reduced] Phase 16 enables fail-closed production mutual TLS for Laravel-to-correlation-worker health and cutover calls, with CA/hostname verification, client certificates, private keys restricted to root/group `44444`, completion-gated certificate initialization, no production host port, and a semantic CI gate. Live Docker proof used the rebuilt Go service: valid Python and Laravel clients returned 200 while a client without a certificate was rejected with `TLSV13_ALERT_CERTIFICATE_REQUIRED`. Remaining scope includes ingestion-gateway and normalizer-worker HTTP clients, alert-writer-service, incident-builder-service, plus Ollama, Tempo/OTLP, and MinIO where enabled. | `docker-compose*.yml`, `app/Services/InternalMtlsHttpClient.php`, `app/Support/XdrCorrelationCutover.php`, `app/Console/Commands/XdrStranglerStatusCommand.php`, `scripts/xdr_correlation_mtls_compose_validate.py` | High | Ongoing (Codex - normalizer-worker mTLS phase, 2026-08-30) |
