...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-21 done; phase 22 scenario-runner mTLS ongoing) | [Enterprise BLOCKER - reduced] Phase 22 is routing real-mode Laravel scenario ingestion through the existing fail-closed `InternalMtlsHttpClient`, preserving plaintext local mode while enabling CA/hostname/client-certificate verification for HTTPS. Remaining scope is production connector orchestration and the ingestion-gateway server cutover, plus noncanonical validation scripts, Ollama, Tempo/OTLP, and MinIO where enabled. | `app/Jobs/ExecuteScenarioRunJob.php`, `tests/Feature/ScenarioRunnerTest.php` | High | Ongoing (Codex, 2026-08-30) |
