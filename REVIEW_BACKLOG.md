...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 33 ongoing - endpoint shadow-correlation validator mTLS) | [Enterprise BLOCKER - reduced] Phase 33 is adding opt-in fail-closed mTLS to endpoint shadow-correlation service-mode health and correlate requests while preserving offline local simulation. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_endpoint_shadow_correlation_validate.py`, `tests/scripts/test_xdr_endpoint_shadow_correlation_validate.py`, `docs/operations/endpoint-shadow-validation.md` | High | Ongoing |
