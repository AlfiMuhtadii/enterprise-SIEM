...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 37 ongoing - staged-cutover mTLS propagation) | [Enterprise BLOCKER - reduced] Propagate opt-in fail-closed mTLS into the staged-cutover validator's normal shadow and active benchmark subprocesses. The intentionally unreachable plaintext fallback probe and PHP rollback checks remain isolated and receive no client identity. | `scripts/xdr_staged_cutover_validate.py`, `tests/scripts/test_xdr_staged_cutover_validate.py`, `docs/guides/README_XDR_STRANGLER_OPERATIONAL_VALIDATION.md` | High | Ongoing |
