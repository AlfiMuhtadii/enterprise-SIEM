...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-17 done - infrastructure transport, Laravel-to-AI-RAG/correlation, and all normalizer HTTP clients are protected) | [Enterprise BLOCKER - reduced] Phase 17 requires production mTLS on normalizer-worker for Laravel health/replay/resilience and ingestion backpressure traffic. Ingestion now has an explicit client-only mTLS mode, so its normalizer poll is protected without changing the externally consumed ingestion API; successful polls are observable. Normalizer private keys remain root/group `44444`, startup is certificate-init gated, and no normalizer port is published in production. Live rebuilt-service proof returned 200 to CA/hostname-verified Python and Laravel clients, rejected a missing client certificate, recorded 11 authenticated ingestion polls, and retained plaintext ingestion metrics access. Remaining scope includes the ingestion-gateway inbound API and its endpoint/connector clients, alert-writer-service, incident-builder-service, plus Ollama, Tempo/OTLP, and MinIO where enabled. | `docker-compose*.yml`, `services/ingestion-gateway/main.go`, `app/Console/Commands/DlqReplayCommand.php`, `app/Services/ResilienceValidationService.php`, `scripts/xdr_normalizer_mtls_compose_validate.py` | High | Ongoing (Codex - alert-writer-service mTLS phase, 2026-08-30) |
