...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 30 ongoing - fault-injection service transport mTLS) | [Enterprise BLOCKER - reduced] Phase 30 is adding opt-in fail-closed mTLS to the fault-injection tool's ingestion-gateway, normalizer, and alert-writer paths while isolating the internal client identity from Laravel. Local plaintext defaults remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_fault_injection.py`, `tests/scripts/test_xdr_fault_injection.py`, `docs/validation/VALIDATION_BASELINES.md` | High | Ongoing |
