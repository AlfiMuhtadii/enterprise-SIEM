...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-32 done - endpoint-normalization service mode supports mTLS) | [Enterprise BLOCKER - reduced] Endpoint-normalization service mode now uses opt-in fail-closed mTLS for health and normalize requests, while offline local validation remains certificate-independent. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_endpoint_normalization_validate.py`, `tests/scripts/test_xdr_endpoint_normalization_validate.py`, `docs/operations/endpoint-telemetry-validation.md` | High | Proposed (reduced) |
