...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-43 done - stream-bus Pandaproxy TLS and failure status) | [Enterprise BLOCKER - reduced] Topic bootstrap, deterministic demo-alert publishing, and finite stream-bus Redpanda publishing now support private-CA/hostname verification while local plaintext/JSONL defaults remain compatible. Stream-bus failures are counted, return nonzero, and carry `dlq=true`. Remaining scope is Ollama, Tempo/OTLP, and other noncanonical clients where enabled. | `scripts/xdr_{topic_bootstrap,send_demo_alert,stream_bus}.py`, `docs/operations/PANDAPROXY_BOOTSTRAP_TLS.md` | High | Proposed (reduced) |
