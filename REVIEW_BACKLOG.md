...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phases 1-41 done - Pandaproxy bootstrap TLS verification) | [Enterprise BLOCKER - reduced] The canonical topic bootstrap now verifies the private CA and hostname when using the TLS Pandaproxy listener, while its local plaintext default and internal `rpk` path remain compatible. Remaining scope is Ollama, Tempo/OTLP, and other noncanonical clients where enabled. | `scripts/xdr_topic_bootstrap.py`, `tests/xdr_topic_bootstrap/test_xdr_topic_bootstrap.py`, `docs/operations/PANDAPROXY_BOOTSTRAP_TLS.md` | High | Proposed (reduced) |
