...
| Task ID | Title | File / Component | Priority | Status |
|---|---|---|---|---|
| **ENT-SEC-NO-TLS-INTERNAL** (phase 31 ongoing - event-flow validator service mTLS) | [Enterprise BLOCKER - reduced] Phase 31 is adding opt-in fail-closed mTLS to the event-flow validator's alert-writer and incident-builder paths while keeping the client identity off Redpanda REST. Local plaintext defaults remain unchanged. Remaining scope is other noncanonical validation scripts plus Ollama, Tempo/OTLP, and MinIO where enabled. | `scripts/xdr_event_flow_resilience_validate.py`, `tests/scripts/test_xdr_event_flow_resilience_validate.py`, `docs/validation/VALIDATION_BASELINES.md` | High | Ongoing |
