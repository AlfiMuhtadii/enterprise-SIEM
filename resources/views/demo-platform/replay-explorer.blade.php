<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Replay Pipeline Explorer</h2>
    </x-slot>
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-gray-50 border border-gray-200 rounded p-3 text-sm text-gray-700">
            All events are deterministically replayable. ON CONFLICT DO NOTHING guarantees idempotency.
            Showing deterministic scenario runs only.
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-xs text-gray-500 uppercase">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left">Autonomous Remediation</th>
                ...
                <tbody class="divide-y">
                    @forelse($runs as $run)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-green-600">{{ $run->has_autonomous_remediation ? 'Yes' : 'No' }}</td>
                            <td class="px-4 py-2 text-xs text-gray-400">{{ $run->created_at?->format('Y-m-d H:i') }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="px-4 py-4 text-center text-gray-400 text-sm">No deterministic runs yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
